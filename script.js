...
const myApiKey = "99939f8fe9c2afc890d37e351204ef93";
...
function getURL(search, endpoint, name) {
  const baseURL = "https://gateway.marvel.com";
  let endpointURL = endpoint;
  const paramString = formatUrlString(params);
  const url = `${baseURL}${endpointURL}?${paramString}&${search}=${encodeURIComponent(name)}`;
  return url;
}
...
function getResultsName(name) {
  ...
  fetch(url)
    ...
    .catch(err => {
      alert(`Something went wrong: name not found!`)
      $('.loading-icon').addClass('hidden')
    })
}
...
function getEventId(ref) {
  ...
  fetch(url)
    ...
    .catch(err => {
      alert(`Something went wrong: event not found!`)
      $('.loading-icon').addClass('hidden')
    })
...
function getComicId(ref) {
  ...
  fetch(url)
    ...
    .then(responseJSON => getResults(search, responseJSON.data.results[0].id, 1))
    .catch(err => {
      alert(`Something went wrong: comic not found!`)
      $('.loading-icon').addClass('hidden')
    })
}

function callAPI(x) {
  let w = window.location.href.split('/')
    w = w.pop();
    if (w === 'by-name.html') {
      getResultsName(x);
    } else if (w === 'by-event.html') {
      getEventId(x);
    } else {
      getComicId(x);
    }
...
function attachSubmitListener(formName) {
...
function watchForm() {
  attachSubmitListener('.byName');
  attachSubmitListener('.byEvent');
  attachSubmitListener('.byComic');
  hamburgerMenu();
}
